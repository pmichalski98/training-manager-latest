...
const Navigation = () => {
  return (
    <nav
      className="shadow-3xl fixed bottom-0 z-10 w-full  overflow-hidden bg-nav/90 shadow-white
       transition
...
function NavItem({ title, href }: NavItemI) {
  ...
  const classes = twMerge(
    "cursor-pointer flex flex-col md:flex-row py-4 md:items-center left-0 ml-6  md:absolute  md:flex gap-2 opacity-75 hover:text-primary hover:opacity-100",
    active && "text-primary opacity-100"
  );
  return (
    <li>
      <Link href={href} className={classes}>
        <NavIcon name={title} className="mx-auto md:h-8 md:w-8" />
        <span className="relative md:ml-5 md:text-xl">{title}</span>
      </Link>
    </li>
  );
}
